...
export default function WhatsAppButton() {
  ...
  // Distinguer "en cours de chargement" vs "définitivement hors ligne" : sinon
  // le badge orange s'affiche pendant le fetch initial et donne à tort
  // l'impression que personne n'est joignable — alors qu'on ne sait pas encore.
  const statusKnown = businessStatus !== undefined;
  ...
  // Message WhatsApp adapté selon la disponibilité — localisé (clés common.json)
  const whatsappMessage = isAvailable
    ? t("whatsapp_message")
    : t("whatsapp_message_offline");
  ...
  return (
    <div className="fixed bottom-6 left-6 z-[60]">
      ...
      >
        ...
        {/* Indicateur en ligne / hors ligne — visible dès le 1er render, transition douce */}
        <span className={`absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full border-2 border-white transition-colors duration-300 ${
          !statusKnown ? "bg-gray-400" : isAvailable ? "bg-green-500" : "bg-orange-400"
        }`}>
          {statusKnown && isAvailable && <span className="absolute inset-0 rounded-full bg-green-500 animate-ping opacity-40" />}
        </span>
